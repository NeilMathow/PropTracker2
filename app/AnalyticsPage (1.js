  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        maxWidth: "520px",
        width: "100%",
        boxShadow: "0 0 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(249,115,22,0.1)",
      }}>
  ...
  return (
    <div style={{
      ...
      <div
        ...
        style={{
          background: "var(--surface)",
          border: `1px solid ${dragActive ? "#f97316" : "rgba(249,115,22,0.25)"}`,
          borderRadius: "20px",
          padding: "44px 52px",
          maxWidth: "520px",
          width: "100%",
...
function CoachingInsights({ stats: s }) {
  ...
  // Strengths — top items
  const strengths = [];
  if (s.wr >= 55) strengths.push({ label: "Win Rate", desc: `${s.wr.toFixed(1)}% — consistent edge on direction` });
  if (s.pf >= 1.5) strengths.push({ label: "Profit Factor", desc: `${s.pf.toFixed(2)} — winners dominating losers` });
  if (wlRatio >= 1.3) strengths.push({ label: "Risk/Reward", desc: `${wlRatio.toFixed(2)}x — good trade management` });
  if (dayWR >= 65) strengths.push({ label: "Day Discipline", desc: `${dayWR.toFixed(0)}% green days — consistent sessions` });
  if (feeImpact < 6) strengths.push({ label: "Low Fee Drag", desc: `Only ${feeImpact.toFixed(1)}% of gross — selective trading` });
  if (s.exp > 40) strengths.push({ label: "Positive Expectancy", desc: `$${s.exp.toFixed(0)} per trade — strategy has real edge` });
  if (strengths.length === 0) strengths.push({ label: "Building Your Record", desc: "Keep trading consistently — your edge will show over time" });
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "14px", overflow: "hidden", marginBottom: "12px", transition: "border-color 0.2s, box-shadow 0.2s" }}
      ...
      <div style={{ padding: "1.25rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr 1fr", gap: "14px", alignItems: "start" }}>

          {/* ── Col 1: Score + ratings ── */}
          <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderBottom: "2px solid #f97316", borderRadius: "12px", padding: "18px" }}>
            ...
            <div style={{ fontSize: "9px", color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "10px", fontWeight: 600 }}>Performance Ratings</div>
          ...
          <div style={{ background: "var(--surface2)", border: "1px solid rgba(0,217,126,0.2)", borderLeft: "3px solid #00d97e", borderRadius: "12px", padding: "16px" }}>
            <div style={{ fontSize: "9px", fontWeight: 700, color: "#00d97e", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "14px" }}>✓ Strengths</div>
            {strengths.slice(0, 4).map((item, i) => (
              <div key={i} style={{ marginBottom: "12px", paddingBottom: "12px", borderBottom: i < strengths.slice(0,4).length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                <div style={{ fontSize: "12px", fontWeight: 700, color: "#00d97e", marginBottom: "3px" }}>{item.label}</div>
          ...
          {/* ── Col 3: Focus areas ── */}
          <div style={{ background: "var(--surface2)", border: "1px solid rgba(249,115,22,0.2)", borderLeft: "3px solid #f97316", borderRadius: "12px", padding: "16px" }}>
            <div style={{ fontSize: "9px", fontWeight: 700, color: "#f97316", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "14px" }}>⚡ Focus Areas</div>
            {focusAreas.slice(0, 4).map((item, i) => (
              <div key={i} style={{ marginBottom: "12px", paddingBottom: "12px", borderBottom: i < focusAreas.slice(0,4).length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                <div style={{ fontSize: "12px", fontWeight: 700, color: "#f97316", marginBottom: "3px" }}>{item.label}</div>
                <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>{item.desc}</div>
              </div>
            ))}
          </div>
...
function InfoCard({ label, value, cls, right }) {
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderLeft: "3px solid #f97316", borderRadius: "10px", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "box-shadow 0.2s", minHeight: "72px" }}
      onMouseEnter={e => e.currentTarget.style.boxShadow = "0 0 20px rgba(249,115,22,0.1)"}
      onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}>
      <div>
...
function Dashboard({ allTrades, onReset }) {
  ...
  const firmColors = firmKeys.map((_, i) => FC[i % FC.length]);
          ...
          <div style={{ padding: "1rem 1.25rem" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: "14px" }}>
              <div style={{ position: "relative", width: "90px", height: "90px", flexShrink: 0 }}>
                <DonutChart data={firmKeys.map(k => Math.abs(+firms[k].pnl.toFixed(2)))} colors={firmColors} size={90} />
              </div>
              <div style={{ flex: 1, overflow: "hidden" }}>
                {firmKeys.map((k, i) => (
                  <div key={k} style={{ display: "flex", alignItems: "center", gap: "7px", marginBottom: "7px" }}>
                    <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: firmColors[i], flexShrink: 0 }} />
            ...
            <tbody>
              {[...allTrades].sort((a,b)=>tradeTab==="top"?b._pnl-a._pnl:a._pnl-b._pnl).slice(0,10).map((t,i)=>(
                <tr key={i}>
                  ...
                  <td><span className="type">{t.ContractName||"—"}</span></td>
...
export default function AnalyticsPage({ uploadRef }) {
  ...
  const handleFiles = (files) => {
    Array.from(files).forEach(f => {
      ...
      reader.onload = e => {
        setPendingQueue(prev => [...prev, { file: f, rawText: e.target.result }]);
      };
      reader.readAsText(f);
    });
  };
